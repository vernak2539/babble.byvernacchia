...
module Jekyll
  ...
  class Image < Liquid::Tag
	...
	src_values    = generate_image(instance, site.source, site.dest, settings['source'], settings['output'])
	generated_src = src_values[0]
	finalWidth    = src_values[1]
	finalHeight   = src_values[2]

      # Return the markup!
      "<div style='width:#{finalWidth}px; height:#{finalHeight}px;' class='img-centered'><img src=\"#{generated_src}\" #{html_attr_string} width=\"#{finalWidth}\" height=\"#{finalHeight}\"></div>"
    end
    ...
    def generate_image(instance, site_source, site_dest, image_source, image_dest)
      ...
      values = Array.[](Pathname.new(File.join('/', image_dest, image_dir, gen_name)).cleanpath, gen_width.round, gen_height.round)
    end
  end
end
