  ...
  var LunrSearch = (function() {
    ...
    LunrSearch.prototype.search = function(query) {
      ...
      if (query.length <= 2) {
        //this.$results.hide();
        this.$entries.empty();
        this.$entries.html('<strong>There are no results for this search</strong>');
      } else {
    ...
    LunrSearch.prototype.displayResults = function(entries) {
      ...
      if (entries.length === 0) {
        ...
        entries = $.map( entries, function( entry ) {
          if( entry.title.length >= maxLength ) {
            entry.title = entry.title.substr( 0, maxLength ) + "...";
          }
          if( !!entry.date ) {
            return entry;
          } else {
        ...
        $entries.append('<strong>Results:</strong>');
